...
# `|| true` here until 2026-09-19: a fold that exited 42 still printed
# "install complete", and --approve mesh-bootstrap then recorded
# written: true with post_memory_md_hash null and no MEMORY.md at all
# (SEED-080 review finding 9, executed as
# bootstrap_failure_recorded_success). Bootstrap's whole job is to prove the
# loop once, live; a step that failed is not a step that can be silent.
if [ -f "$STORE/MEMORY.md.pre-mesh" ]; then
  if ! python3 "$CODE/backfill.py" --commit; then
    echo "FAILED: backfill.py (backfilling $STORE/MEMORY.md.pre-mesh) — bootstrap is NOT complete" >&2
    exit 1
  fi
fi
if ! python3 "$CODE/fold.py"; then
  echo "FAILED: fold.py — the memory loop was never proved on this host; bootstrap is NOT complete" >&2
  exit 1
fi
